      ...
      {!hasProducts ? (
        <div>
          ...
          <div className="flex gap-2 mb-3">
            {PLACEHOLDER_SLOTS.map(({ label, icon, hint }) => (
              <div
                key={label}
                className="flex-1 border-2 border-dashed border-stone-200 rounded-xl p-3 flex flex-col items-center gap-1.5"
              >
                <span className="text-2xl opacity-30">{icon}</span>
                <span className="text-[10px] font-bold text-stone-300 text-center leading-tight">{label}</span>
                <span className="text-[9px] font-medium text-stone-200">{hint}</span>
              </div>
            ))}
            {/* 추가 슬롯 암시 */}
            <div className="flex-1 border-2 border-dashed border-stone-100 rounded-xl p-3 flex flex-col items-center justify-center gap-1">
              ...
              <span className="text-[9px] font-bold text-stone-200">더 추가</span>
            </div>
          </div>
          <p className="text-[11px] font-medium text-stone-400 text-center mb-2.5">
            분유·우유·영양제를 담으면 여기서 한눈에 관리할 수 있어요
          </p>
          <Link
            href="/search"
            className="flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-extrabold text-orange-500 border-2 border-dashed border-orange-200 rounded-xl hover:bg-orange-50 transition-colors"
          >
        ...
        <div className="flex flex-col gap-2">
          ...
          {routineProducts.map((p) => {
            ...
            return (
              <div key={p.id} className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-bold text-stone-800 text-[13px] truncate">{p.product_name}</p>
          ...
          {supplementProducts.map((p) => {
            ...
            const servingText =
              p.daily_serving_count && p.amount_per_serving && p.serving_unit
                ...
                : null
            return (
              <div key={p.id} className="bg-white border border-stone-100 rounded-xl p-3 flex items-center gap-3">
          ...
          <Link
            href="/search"
            className="flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-extrabold text-orange-500 border-2 border-dashed border-orange-200 rounded-xl hover:bg-orange-50 transition-colors mt-1"
          >
            <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
              <circle cx="11" cy="11" r="7" /><line x1="20" y1="20" x2="16.1" y2="16.1" />
            </svg>
